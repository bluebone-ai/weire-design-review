...
- 强制登记标签、角标、红点、次数、指示器、提示和微文案，检查元素必要性、删除影响与完整状态生命周期
- 静态截图无法证明的点击、消失或重复规则只进入待验证；Figma、PRD、录屏或实现证据确认后才参与扣分
...
- 通过脚本校验报告结构并生成确定性评分
- 总分同时受维度质量和全局问题严重度约束，取两套结果中的较低值，避免低权重维度稀释真实问题
- 重大、一般问题的数量和对本轮目标的直接影响会进入开发准入校准
- 自动输出开发准入结论：85 分为正常开发线，70–84 分需修改后进入，低于 70 分先调整设计
